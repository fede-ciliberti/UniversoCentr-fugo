...
En las etapas previas descubrimos que si nuestra velocidad angular $\omega_{4D}(t)$ decae como $1/R^2$ para conservar el momento angular del cosmos en el Bulk (ver [`conservacion_momento_inercia_geff.md`](conservacion_momento_inercia_geff.md) para la derivación completa de la estabilidad de $G_{eff}$), la densidad centrífuga decae demasiado rápido ($a^{-5}$), comportándose como un fluido súper rígido ($w = +2/3$) incapaz de acelerar el universo en la actualidad. 
